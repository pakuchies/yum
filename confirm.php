...
/*
ファイルパス
C:\xampp\htdocs\member\confirm.php
ファイル名
confirm.php
アクセスURL
http://localhost/member/confirm.php
 */
...
case "confirm"://新規登録
    ...
    $template = ( $err_check === true )?'confirm.tpl':'regist.tpl';
...
case "complete":
    ...
    if($res === true ){
        //登録成功時は完成ページへ
        header("Location: http://localhost:88/member/complete.php");
        exit;
    }else{
        //登録失敗時は登録画面に戻る
        $template = "regist.tpl";
